...
function unRegister(iden) {
  if (confirm("Do you want to unregister for this event?")) {
    ...
    var ref = db.collection("database2/course2/TeeTimes").doc(iden);
    ref.get().then((doc) => {
...
function register(iden) {
  ...
  var docRef = db.collection("database2/course2/TeeTimes").doc(iden);
  docRef
    .get()
    .then(function (doc) {
      if (doc.exists) {
...
function write() {
  ...
  var tomorrow = new Date(dayOne);
  tomorrow.setDate(tomorrow.getDate() + 1);
  var ref = db.collection("database2/course2/TeeTimes").doc(identity);
  ref.get().then((doc) => {
    var date = new Date(doc.data().start * 1000);
    ...
    var dat =
      ...
      "-" +
      rawDay.substr(0, 2);
    var cardDate = Date.parse(dat.toString() + "T" + hours.toString() + ":" + minutes.toString() + ":00");
    var TorF = Date.today().compareTo(cardDate);
    console.log(cardDate);
    console.log(Date.today());
    console.log(rawDay);
    console.log(dat.toString() + "T" + hours.toString() + ":" + minutes.toString() + ":00");
    console.log(cardDate.getTime() / 1000);
    console.log(tomorrow.getTime() / 1000);
    var tom = tomorrow.getTime() / 1000;
    var cardD = cardDate.getTime() / 1000;
  ...
  //   console.log("cancelled");
  // }
  // var ref = db.collection("database2/course2/lessons").doc(identity);
  // ref.update({
  //   registered: firebase.firestore.FieldValue.increment(1),
  //   signedUp: firebase.firestore.FieldValue.arrayUnion(
  //     "(" + name + ") " + email
